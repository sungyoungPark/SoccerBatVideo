...
class JsonManager {
    ...
    func getJson(completed: @escaping (Observable<[FeedData?]>) -> Void) {
    
        if let URL = URL(string: apiURL + "/?token=\(myToken)") {
                ...
                if let data = data {
                    do {
                        ...
                        if let res = jsonObject["response"] as? [[String : Any]]{
                            ...
                            print("success",URL)
                            print(classes.count)
                            completed( Observable.just(classes))
                        }
                    }
                    catch {
                        print("error",URL)
    ...
    func getJson2() -> Observable<[FeedData?]> {
